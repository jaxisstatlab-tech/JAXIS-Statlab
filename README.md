...
- **Dashdark X Sidebar-First Shell & Zero Desktop Topbar Architecture**: Desktop main canvas operates with zero horizontal topbar ($y = 0$), maximizing PageHeader vertical space. User identity, role display tags, profile routing, and sign-out controls live directly in the Sidebar footer profile card (`Sidebar.tsx`) with full collapsible icon rail support (`w-[17.5rem]` $\leftrightarrow$ `w-[4.25rem]`) and Windows Chromium native scrollbar elimination.
- **Client Dashboard Dashdark X Bento Upgrade**: 2:1 asymmetric bento arrangement with 8-col Active Research Journey hero featuring an integrated Active Milestone Mission Panel (APA 7th edition formatting standard, operational mission context, and dual-audit credentials), 5-stage precision pipeline track, and 4-col stacked consultation & DefenseLab cards.
- **Real-Time Notification Event Engine & Catalog (Module 16 & Docs 09)**: Cross-role event dispatcher (`dispatchRealtimeNotification`) covering all operational mutations, fresh account onboarding alerts (`ensureFreshAccountNotifications`), in-flight study milestone auto-backfill (`ensureProjectLifecycleNotifications`), chronological descending sorting, and quiet-when-zero live badging standard.
...
- **Staff Attendance & Duty Governance (Module 18)**: Sidebar and mobile live duty clock widget with 0ms optimistic transitions, RSC pre-loading, 14-hour anti-runaway shift safety auto-close, automated meal break deductions, and Segregation of Duties shift correction approval workflows.
- **Corporate Payroll & Itemized Payslips (Module 19)**: CEO executive compensation rate matrix, package tier rates, bespoke PhD consultant overrides, semi-monthly batch calculation engine, self-service e-wallet/bank settlement profiles, 1-click treasury disbursements, and official QR-stamped payslip vouchers.
