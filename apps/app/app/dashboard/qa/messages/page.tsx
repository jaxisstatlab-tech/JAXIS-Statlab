...
import { QaMessagesClient } from "./QaMessagesClient";
...
export default async function QaMessagesPage({ searchParams }: PageProps) {
  ...
  if (targetProjectId) {
    try {
      ...
      console.error("Failed to prefetch initial QA project messages on server:", err);
    }
  }
    ...
    <Suspense
      fallback={
        <div className="h-full flex-1 flex items-center justify-center">
          <LoadingState variant="page" label="Loading QA messages..." />
        </div>
      }
    >
      <QaMessagesClient
